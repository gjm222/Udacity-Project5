# -*- coding: utf-8 -*-
"""
Created on Fri Apr 14 06:15:56 2017
...
def find_cars(img, ystart, ystop, scale, svc, X_scaler, orient, pix_per_cell, cell_per_block, spatial_size, hist_bins):
    ...
    heatmap = np.zeros_like(img[:,:,0]).astype(np.float) #Init heatmap
    img = img.astype(np.float32)/255  #Normaize image
    ...
    for xb in range(nxsteps):
        for yb in range(nysteps):
            ...
            #print('h=', hist_features.shape)
            #print('s=', spatial_features.shape)
...
def features_for_vis(image, cspace='RGB', orient=9, pix_per_cell=8, cell_per_block=2, hog_channel=0):
    if cspace != 'RGB':
        if cspace == 'HSV':
            feature_image = cv2.cvtColor(image, cv2.COLOR_RGB2HSV)
        elif cspace == 'LUV':
